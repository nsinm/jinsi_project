...
class RepastAction extends WapAction
{
	...
	private function getDishCompany($cid, $cache = true)
	{
		...
		if ($cache && !empty($DishC)) {
			...
			if (!empty($DishC)) {
				...
				$DishC['etimestr'] = 0 < $DishC['endtime'] ? date('H:i', $DishC['endtime']) : '23:59';
			}
			else {
	...
	private function getDishMainCompany($cache = true)
	{
		...
		if ($cache && !empty($mDishC)) {
			...
			if (!empty($mDishC)) {
				...
				$mDishC['etimestr'] = 0 < $mDishC['endtime'] ? date('H:i', $mDishC['endtime']) : '23:59';
			}
			else {
	...
	public function payReturn()
	{
		...
		if (isset($_GET['nohandle'])) {
			...
			$model->sendTempMsg('OPENTM202521011', array('href' => $siteurl . U('Repast/myOrders', array('token' => $order['token'], 'wecha_id' => $order['wecha_id'], 'cid' => $order['cid'])), 'wecha_id' => $order['wecha_id'], 'first' => '餐饮订餐交易提醒', 'keyword1' => $orderid, 'keyword2' => date('Y年m月d日H时i分s秒'), 'remark' => '支付成功，感谢您的光临，欢迎下次再次光临！'));
