...
class MessageAction extends UserAction{
	...
	public function tosend24All(){
		if (IS_GET){
			...
			$apiOauth 	= new apiOauth();
			$access_token 	= $apiOauth->update_authorizer_access_token($this->thisWxUser['appid']);
			$user_info = M('Wechat_group_list')->where(array('g_id'=>$info['groupid']))->select();	
			if ($access_token){
				//OPENID就发送给个人
				...
				//print_r($openid);
				$img_id = explode(',',$info['imgids']);
				$temp_arr = array();
				$img_arr = array();
				foreach($img_id as $k=>$v){
					$img_info = M('img')->where(array('id'=>$v))->find();
			//		print_r($img_info);
					$pic_url = str_replace('{siteUrl}','http://182.92.222.144',$img_info['url']);
					$img_arr[] = array(
						"title"=>"title".$k,
					...
					$temp_arr[] = array($img_info['text'],$img_info['keyword']);
				}
				//print_r($img_arr);
				foreach($openid as $v){
					
					$mess_arr = json_encode(array('touser'=>$v,'msgtype'=>'news','news'=>array("articles"=>$img_arr)));
					$mess_arr = str_replace('{wechat_id}',$v,$mess_arr);
					foreach($temp_arr as $tk=>$tv){
						$mess_arr = str_replace("title".$tk,$tv[0],$mess_arr);
						$mess_arr = str_replace("description".$tk,$tv[1],$mess_arr);
		
					}
					//echo $mess_arr;exit;
					$sendrt=$this->curl24Post('https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.$access_token,$mess_arr);
				}
				M('Send_message')->where(array('id'=>$id))->save(array('status'=>1,'time'=>time()));
	...
	function curl24Post($url, $data,$showError=1){
		$ch = curl_init();
