...
class VoteimgAction extends UserAction
{
	...
	public function add_voteimg()
	{
		if (IS_POST) {
			...
			$action_data['limit_vote_item'] = (int) $_POST['limit_vote_item'];
	...
	public function add_item()
	{
		if (IS_POST) {
			...
			if (18 < strlen($_POST['vote_title'])) {
				$this->error('选项标题不超过6个汉字');
				exit();
			}
			...
			$img_data['introduction'] = trim($_POST['introduction']);
	...
	public function edit_item()
	{
		if (IS_POST) {
			...
			$img_data = array();
			$img_data['vote_title'] = trim($_POST['vote_title']);
			$img_data['introduction'] = $_POST['introduction'];
