...
<?php if(is_array($class)): $i = 0; $__LIST__ = $class;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($c["name"]); ?></td><td class="norightborder"><?php if($c['sub'] == NULL): ?><a href="###" onclick="returnHomepage(<?php echo ($c["id"]); ?>,'<?php echo ($c["name"]); ?>',<?php echo ($c["pc_cat_id"]); ?>)">选中</a><?php else: ?><a href="###" onclick="returnHomepage(<?php echo ($c["id"]); ?>,'<?php echo ($c["name"]); ?>,<?php echo ($c["pc_cat_id"]); ?>')">选中</a> &nbsp; <a href="?g=User&m=Img&a=editClass&id=<?php echo ($c["id"]); ?>">详情</a><?php endif; ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
</table>
...
// 返回数据到主页面
function returnHomepage(url,n,pc_cid){
	...
	if(pc_cid){
		var dom3 = origin.document.getElementById('pc_show');
		dom3.style.display = 'table-row';
		var dom4 = origin.document.getElementById('pc_cat_id');
		dom4.value = pc_cid;
	}else{
		var dom3 = origin.document.getElementById('pc_show');
