...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
	<td><?php echo ($list["grabber_nickname"]); ?></td>
	<td><?php echo ($list["action_name"]); ?></td>
	<td><?php echo ($list["money"]); ?></td>
	<!--<td><?php echo ($list["code"]); ?></td>-->
	<td>
		<?php if($list["isgrabbed"] == '2'): ?><font color="">已领取</font>
		<?php elseif($list["isgrabbed"] == '1'): ?>
			抢到未领取
		<?php else: ?>
			未抢到<?php endif; ?>
	</td>
	<td class="norightborder">
		<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Hongbao/del_packet',array('token'=>$token,'id'=>$list['grabber_id']));?>');">删除</a>
    </td>
