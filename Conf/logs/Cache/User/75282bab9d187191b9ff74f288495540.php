...
<td>
  <input type="text" name="limit_vote" value="<?php if($vo["limit_vote"] == ''): ?>0<?php else: echo ($vo["limit_vote"]); endif; ?>"  class="px">&nbsp;&nbsp;&nbsp;&nbsp;(限制每个用户每个活动总的投票数，超过不让投票,<span style="color:red;">0表示不限制</span>,默认为0,请输入整数)
</td>
</tr>
<tr>
<th><span class="red">*</span> 限制每天投票数：</th>
<td>
  <input type="text" name="limit_vote_day" value="<?php if($vo["limit_vote_day"] == ''): ?>0<?php else: echo ($vo["limit_vote_day"]); endif; ?>"  class="px">&nbsp;&nbsp;&nbsp;&nbsp;(限制每个用户每个活动每天的投票数，超过不让投票,<span style="color:red;">0表示不限制</span>,默认为0,请输入整数)
</td>
</tr>
<tr>
<th><span class="red">*</span> 限制选项每天票数：</th>
<td>
  <input type="text" name="limit_vote_item" value="<?php if($vo["limit_vote_item"] == ''): ?>0<?php else: echo ($vo["limit_vote_item"]); endif; ?>"  class="px">&nbsp;&nbsp;&nbsp;&nbsp;(限制每个选项每天获得的票数,<span style="color:red;">0表示不限制</span>,1表示每个用户每天只能给该选项投1票,默认为0,请输入整数)
</td>
</tr>
<tr>
<th> 电话：</th>
...
<th>页面展示：</th>
<td>
  <input type="radio" name="page_type" value="waterfall" <?php if($vo["page_type"] == 'waterfall' or $vo["page_type"] == ''): ?>checked<?php endif; ?>>
  自动加载形式
  <input type="radio" name="page_type" value="page" <?php if($vo["page_type"] == 'page'): ?>checked<?php endif; ?>>分页形式  &nbsp;&nbsp;&nbsp;&nbsp;(手机端投票选项加载方式)
</td>
</tr>
<tr>
<th>是否开启：</th>
...
function save_voteimg(){
	...
	if(limit_vote == ''){
		msg += '限制投票数不能为空\n';
	}else if(!reg.test(limit_vote)){
		msg += '限制投票数请输入正整数\n';
	}	
	if(limit_vote_day == ''){
		msg += '限制每天投票数不能为空\n';
	}else if(!reg.test(limit_vote_day)){
		msg += '限制每天投票数请输入正整数\n';
	}
	if(!reg.test(limit_vote_item)){
		msg += '限制选项每天票数请输入整数\n';
	}
	if(msg){
		alert(msg);
